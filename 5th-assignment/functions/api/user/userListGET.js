...
module.exports = async (req, res) => {
  ...
  // try문 안에서 우리의 로직을 실행합니다.
    try {
        ...
        const posts = await postDB.getPostByUserIds(client,userIds)

        for(let i =0 ;i<users.length;i++){
          users[i].posts = _.filter(posts,(o)=>o.userId === users[i].id);
        }

        if(!users) return res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, responseMessage.NULL_VALUE))
        // 성공적으로 users를 가져왔다면, response를 보내줍니다.
        res.status(statusCode.OK).send(util.success(statusCode.OK, responseMessage.READ_ALL_USERS_SUCCESS, {users,posts}));
